...
class N_LinUCBAlgorithm:
	def __init__(self,attributes,G,P,parameter, node_list, seed_size, oracle, dimension, alpha,  lambda_ , feedback = 'edge'):
		...
		self.users = []  #Nodes
		self.Theta = np.zeros((int(node_list), int(dimension)))
...
class LinUCBAlgorithm:
	def __init__(self, attributes, G, seed_size, oracle, dimension, alpha,  lambda_ , FeatureDic, feedback = 'edge'):
		self.G = G
		self.oracle = oracle
		self.seed_size = seed_size
		self.attributes=attributes
