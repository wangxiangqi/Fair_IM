## Modified by Xiangqi Wang, 06/06/2023
## Added support of Maximin fairness
import time
import os
import pickle 
import datetime
...
class simulateOnlineData:
    ...
    def runAlgorithms(self, algorithms):
        ...
        self.resultRecord()
        optS=pickle.load(open('E:/summer_intern/Hua_zheng_Wang/Fair_IM/IMFB-KDD2019-master/IMFB-KDD2019-master/fair_influmax_code/optimal_set.pkl', 'rb'), encoding='latin1')

        for iter_ in range(self.iterations):
            #在这里的求optimal_reward的过程中应该要引入class信息才能得到一个maximin的reward。
            #求IC Model中要引入
            ...
            for alg_name, alg in list(algorithms.items()): 
                S = alg.decide() 
                reward, live_nodes, live_edges = runICmodel_n(G, S, self.class_feature, self.TrueP)
...
if __name__ == '__main__':
    ...
    for key, value in parameter.items():
        if key in mapping.keys():
            parameter_1[mapping[key]]=value
    parameter=parameter_1
    feature_dic = pickle.load(open(edge_feature_address, 'rb'), encoding='latin1')
    feature_dic_1={}
    for key, value in feature_dic.items():
        if key[0] and key[1] in mapping.keys():
            feature_dic_1[(mapping[key[0]],mapping[key[1]])]=value
    feature_dic=feature_dic_1
    class_feature = pickle.load(open(class_feature_address, 'rb'),encoding='latin1')
    class_feature_1={}
    for key, value in class_feature.items():
        if key in mapping.keys():
            class_feature_1[mapping[key]]=value[1]
    class_feature=class_feature_1
    print(class_feature)
    P = nx.DiGraph()
    for (u,v) in G.edges():
        if (u,v) in prob.keys():
