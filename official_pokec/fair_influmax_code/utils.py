...
def greedy(items, budget, f):
    ...
    for u in items:
        ...
        #print(u)
            upper_bounds.append((-f(set([u])), u))
    #print("loaded upper bounds")
    heapq.heapify(upper_bounds)
    ...
    while len(S) < budget:
        ...
        # We take epsilon as 0.1 yet we have to tune the parameter
        #print(new_val+upper_bounds[0][0] + 100)
        
        # If we take pokec dataset, we set the threshold as 1200
        # If we take german dataset, we set the threshold as 250
        if new_val >= -upper_bounds[0][0] - 1250:
            S.add(u)
            starting_objective = new_total
        else:
            heapq.heappush(upper_bounds, (-new_val, u))
