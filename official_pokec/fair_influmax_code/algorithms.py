...
def make_welfare(f, targets,alpha):
    def wel(x, *args):
...
def mirror_sp(x, grad_oracle, k, group_indicator, group_targets, num_iter, step_size = 50, batch_size = 1000, verbose=False):
    '''
...
#    startstep = stepsize
    ...
    for t in range(num_iter-1):
        ...
        iter_targets = group_targets - val_oracle((1/num_iter)*x[0:t].sum(axis=0), 5) 
...
#        stepsize = startstep/np.sqrt(2*t+1)
        if solver == 'md':
            ...
            m = mirror_sp((1./(t+1))*x[0:(t+1)].sum(axis=0), grad_oracle, k, group_indicator, iter_targets, num_iter=5)
            #m=m.astype('float')
...
def fw(grad_oracle, val_oracle, threshold, k, group_indicator, group_targets, num_iter, stepsize= 1):
    ...
    x = np.zeros((num_iter, group_indicator.shape[0]))
    for t in range(num_iter-1):
        grad = grad_oracle((1./num_iter)*x[0:t].sum(axis=0), 5).sum(axis=1)
...
#        print(target)
        ...
        vals = val_oracle(x.mean(axis=0), 5)
        if vals.min() > target:
            lb = target
        else:
            ub = target
